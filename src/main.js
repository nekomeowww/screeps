...
//tower的运行
    ...
    if(roomEnergy >= 300){
        ...
        if(harvesters.length < minHarvester) {
            if(roomEnergy < 300) {
                ...
                if(roomEnergy >= 300 && roomEnergy < 400) {
                    var newNameHarvesters = Game.spawns['Factory01'].createCreep([WORK,CARRY,CARRY,CARRY,MOVE], nameCreeps, {role: 'harvester'});
                    console.log('正在建造新的开采虫：' + newNameHarvesters);
                }
                else {
                    if(roomEnergy >= 400) {
                        var newNameHarvesters = Game.spawns['Factory01'].createCreep([WORK,WORK,CARRY,CARRY,CARRY,MOVE], nameCreeps, {role: 'harvester'});
                        console.log('正在建造新的开采虫：' + newNameHarvesters);
                    }
                }
            }
        ...
        if(harvesters.length > minHarvester && upgraders.length < minUpgrader) {
            var upgraders = _.filter(Game.creeps, (creep) => creep.memory.role == 'upgrader');
            if(roomEnergy < 300 && roomEnergy >= 250) {
                var newNameUpgraders = Game.spawns['Factory01'].createCreep([WORK,CARRY,CARRY,MOVE], nameCreeps, {role: 'upgrader'});
                console.log('正在建造新的升级虫：' + newNameUpgraders);
            }
            else {
                if(roomEnergy > 400) {
                    var newNameUpgraders = Game.spawns['Factory01'].createCreep([WORK,WORK,CARRY,CARRY,CARRY,MOVE], nameCreeps, {role: 'upgrader'});
                    console.log('正在建造新的升级虫：' + newNameUpgraders);
                }
            }
        }

        if(harvesters.length > minHarvester && constructionSites.length > 0) {
            var builders = _.filter(Game.creeps, (creep) => creep.memory.role == 'builder');
            if(roomEnergy < 300 && builders.length < minBuilder) {
